...
BEGIN
    ...
    SELECT 
        ...
        (SELECT ap.price FROM asset_prices ap 
         WHERE ap.asset_id = a.id AND ap.trade_date = CURRENT_DATE 
         ORDER BY ap.trade_date DESC LIMIT 1) AS today_price,
        (SELECT ap.trade_date FROM asset_prices ap 
         WHERE ap.asset_id = a.id AND ap.trade_date = CURRENT_DATE 
         ORDER BY ap.trade_date DESC LIMIT 1) AS today_date,
        (SELECT ap.price FROM asset_prices ap 
         WHERE ap.asset_id = a.id AND ap.trade_date = CURRENT_DATE - 1 
        ...
        COALESCE(
            (SELECT ap.price FROM asset_prices ap 
             WHERE ap.asset_id = a.id AND ap.trade_date IN (CURRENT_DATE, CURRENT_DATE - 1) 
