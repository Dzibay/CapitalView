-- Внешние ключи для portfolio_asset_daily_values и portfolio_daily_values.
-- Если таблица ещё называется portfolio_daily_positions — сначала
-- migrate_rename_portfolio_daily_positions_to_asset_daily_values.sql
...
UPDATE portfolio_asset_daily_values pdp
...
DELETE FROM portfolio_asset_daily_values pdp
...
ALTER TABLE portfolio_asset_daily_values
  DROP CONSTRAINT IF EXISTS portfolio_asset_daily_values_portfolio_asset_id_fkey;
ALTER TABLE portfolio_asset_daily_values
  DROP CONSTRAINT IF EXISTS portfolio_asset_daily_values_portfolio_id_fkey;
ALTER TABLE portfolio_daily_values
  DROP CONSTRAINT IF EXISTS portfolio_daily_values_portfolio_id_fkey;

ALTER TABLE portfolio_asset_daily_values
  ADD CONSTRAINT portfolio_asset_daily_values_portfolio_asset_id_fkey
  FOREIGN KEY (portfolio_asset_id) REFERENCES portfolio_assets(id) ON DELETE CASCADE;
ALTER TABLE portfolio_asset_daily_values
  ADD CONSTRAINT portfolio_asset_daily_values_portfolio_id_fkey
